    ...
    * **Trace to be verified**
    ...
    def _populate_pids(self, run):
        ...
        if len(self._execnames) == 1:
            return functions.get_pids_for_process(run, self._execnames[0])

        pids = []

        for proc in self._execnames:
            pids += functions.get_pids_for_process(run, proc)
    ...
    def _generate_matrix(self, run, reference_run, aggfunc):
        ...
        for (ref_result, test_result) in agg_pair_gen:
            ...
            corr = Correlator(
                ref_result,
                test_result,
                corrfunc=functions.binary_correlate,
                filter_gaps=True)
